...
<img src="oci-boot-volume.png" alt="부트 볼륨의 부트 볼륨 분리 버튼" width="800">
...
<img src="oci-block-volume-connect.png" alt="블록 볼륨 연결 탭에 표시된 연결할 수 있는 볼륨들" width="600">
...
<img src="oci-connected-block-volume.png" alt="연결된 블록 볼륨의 iSCSI 명령 및 정보 버튼" width="700">

<img src="oci-connected-block-volume-iscsi-command.png" alt="iSCSI 명령어" width="500">
...
<img src="ssh-connect-success.png" alt="터미널에서 iSCSI 명령어 실행 후 볼륨 연결이 성공한 모습" width="700">

`sudo fdisk -l` 명령어로 연결된 볼륨 목록을 확인할 수 있다.

<img src="connected-volume-list.png" alt="터미널에 표시된 연결된 볼륨 목록" width="600">
...
<img src="connected-volume-list-dev-sdb1.png" alt="터미널에 표시된 연결된 볼륨 목록의 sdb1" width="600">

연결된 볼륨 목록 중 리눅스 파일시스템인 `sdb1`에 접근할 것이다.  
마운트할 디렉토리를 만들고 명령어 `sudo mount 디스크주소 디렉토리이름주소`로 디렉토리 마운트를 해준다.

<img src="mount-directory.png" alt="터미널에서 디렉토리 마운트하는 모습" width="700">

만들어둔 디렉토리에 들어가면 `sdb1` 데이터에 접근할 수 있다.  
SSH 접속이 안되는 문제를 확인하기 위해서 `.ssh` 디렉토리의 `authorized_keys` 파일을 확인해봤다.

<img src="authorized_keys.png" alt="터미널에서 authorized_keys 파일을 확인하는 모습" width="700">
...
<img src="oci-issue-mail.png" alt="OCI로부터 온 이슈 메일" width="700">

다시 SSH 접속할 수 있도록 만들기 위해 새로운 SSH 키를 생성해야 한다.  
`ssh-keygen -t rsa`로 key pair를 생성했다.

<img src="ssh-keygen.png" alt="터미널에서 새로운 SSH 키를 생성하는 모습" width="700">
...
<img src="blkid.png" alt="sbd1의 UUID" width="700">

원하는 디스크의 UUID를 복사해서 `fstab` 파일에 붙여넣어야 한다.  
`sudo vi /etc/fstab` 명령어로 `fstab` 파일을 수정할 수 있다.

<img src="vi-fstab.png" alt="fstab 파일" width="700">

위 파일을 아래처럼 복사한 UUID랑 디렉토리 주소를 넣어서 수정한다.

<img src="vi-fstab-result.png" alt="fstab 파일에 UUID와 디렉토리 주소 추가" width="700">
